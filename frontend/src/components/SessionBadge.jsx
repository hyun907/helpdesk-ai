 ...
 * 찾을 수가 없다.
 *
 * 역할을 <html> 의 data 속성으로 올려 둔다 — 메뉴는 정적 HTML 로 나가므로
 * 서버에서 역할을 알 수 없다. 관리 메뉴를 감추는 판단은 CSS 가 여기 붙은 값을 보고 한다.
 *
 * body 가 아니라 <html> 인 이유: 화면 전환(ClientRouter)이 body 를 통째로 갈아 끼운다.
 * 첫 페인트 전 값을 세우는 BaseLayout 의 인라인 스크립트도 같은 자리를 쓴다 —
 * 저기가 기준이고 여기는 로그인/로그아웃으로 값이 바뀔 때 따라 붙이는 쪽이다.
 */
