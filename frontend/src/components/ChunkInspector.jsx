...
function describe(status) {
  ...
  /**
   * 이 화면의 두 동작(인제스트·검색)은 모두 임베딩 모델을 부른다.
   * 상담과 달리 폴백 사다리가 없어서, 모델 호출이 실패하면 그대로 500 이 올라온다.
   *
   * 가장 흔한 원인은 API 키다. 키가 없어도 애플리케이션은 정상 기동하므로
   * (application.yml 이 sk-not-set 을 자리표시자로 넣는다) 화면만 보고는 알 수가 없고,
   * 백엔드 로그에서 "Incorrect API key provided" 를 찾기 전까지 원인이 드러나지 않는다.
   * 그래서 확답 대신 가장 먼저 볼 곳을 알려 준다 — 다른 원인일 수도 있으므로 단정하지 않는다.
   */
  if (status === 500) {
    return '검색·인제스트가 실패했습니다 (HTTP 500). '
      + 'OPENAI_API_KEY 가 주입되었는지 먼저 확인하세요 — 키가 없어도 서버는 정상 기동합니다. '
      + '자세한 원인은 백엔드 로그에 있습니다.';
  }
