...
**디더링**(체커 한 줄)으로 잇습니다. `.px` 판·버튼·배지가 전부 이렇게 칠해집니다:
...
단계 색은 `--px-bg` 에서 `color-mix` 로 자동 계산됩니다(`--tint` / `--shade` 와 섞음).
그래서 판 색을 무엇으로 바꾸든 같은 계열의 단계가 따라옵니다 — 버튼마다 명암을
직접 지정하지 않습니다.

띠는 도트 한 칸(`--px-u`)씩이고 안쪽 여백 안에 들어가서 본문에 닿지 않습니다.
띠 위에 글자가 앉아도 대비는 낮(7.5~10.5:1)·밤(5.2~9.5:1) 모두 기준을 넘습니다.
`--tint`/`--shade` 비율을 키우면 단계가 도드라지는 대신 글자 바닥이 얼룩집니다.

제목과 브랜드는 글자 아래로 `text-shadow` 를 두 단계 깝니다. **blur 는 반드시 0**
입니다 — 흐린 그림자가 한 픽셀이라도 끼면 픽셀 아트가 아니라 평범한 웹 버튼이 됩니다.
글자 자체에 그라데이션을 넣는 `background-clip: text` 는 쓰지 않습니다.
`text-shadow` 가 그 위를 덮어 두 톤이 보이지 않습니다(확인함).

배경 잔디에도 넓은 얼룩을 깔아 봤지만 뺐습니다. 색차가 ΔE 3 으로 작아도 면적이
넓으면 무늬로 읽히고 타일 반복이 눈에 띕니다. 깊이는 판의 단계 음영으로 충분합니다.
