...
| | 서체 | 쓰는 곳 |
|---|---|---|
| 화면 장식 | **Galmuri11** | 브랜드 · 메뉴 · 제목 · 버튼 · 배지 |
| 읽는 글 | 기존 고딕 스택 16px/1.75 | 정책 원문 · 채팅 답변 · 요약 · 신청 내용 |

정책 원문과 AI 답변은 길게 읽는 글이라 픽셀 폰트를 씌우지 않습니다. 화면은 도트로
보이되 읽어야 하는 문장은 평소 고딕 그대로입니다.

Galmuri11 은 SIL OFL 1.1, 현대 한글 11,172자를 **전부** 포함합니다(검증 완료).
`public/fonts/` 에 직접 담아 둡니다 — CDN 이 죽어도 화면이 깨지지 않습니다.
Regular 493KB + Bold 163KB 이고, 한글 웹폰트라 서브셋 없이는 더 줄지 않습니다.
`font-display: swap` 이라 폰트가 늦게 와도 글자는 먼저 보입니다.

### 2. 픽셀 폰트 크기는 12 의 배수만 쓴다

Galmuri11 은 em 박스가 12도트입니다(upem 1200 / 1도트 100units). **12 · 24 · 36px**
에서만 도트가 화면 픽셀과 1:1 로 맞습니다. 14px·18px 로 두면 힌팅 테이블이 없어서
브라우저가 보정을 못 하고 글자가 흐려집니다. 그래서 조항 제목(h3)은 픽셀 폰트를
쓰지 않습니다 — 12px 로 내리면 본문(16px)보다 작아져 위계가 뒤집히고, 18px 로
올리면 격자가 어긋납니다.

### 3. `--sun` 과 `--mark` 를 바꿔 쓰지 않는다
...
- 아이콘 글자는 Galmuri11 이 가진 것만 씁니다. `⚑ ⌕ ✓ ⚙` 는 없어서 시스템 폰트로
